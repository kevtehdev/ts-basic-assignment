...
function runExerciseThree() {
    ...
    const num1: number = 7; // Första heltalet
    const num2: number = 5; // Andra heltalet
    const sum: number = num1 + num2;
    console.log(`The sum of ${num1} and ${num2} is: ${sum}`);

    // Del B: Dividerar ett udda tal med ett jämnt tal
    const evenNumber: number = 4; // Jämnt tal
    const oddNumber: number = 9;  // Udda tal
    const divisionResult: number = oddNumber / evenNumber;
...
function promptAssignment() {
    while (true) {
        ...
        if (assignmentChoice !== -1) {
            console.log();
            prompt(chalk.dim("Press enter to continue..."));
            console.clear();
        }
    }
}
